...
func ParseConfig(cfg string) {
	if cfg == "" {
		log.Error("config file not specified: use -c $filename")
	}

	if !file.IsExist(cfg) {
		log.Error("config file specified not found:", cfg)
	}
	...
	if err != nil {
		log.Error("read config file", cfg, "error:", err.Error())
	}

	var c GlobalConfig
	err = json.Unmarshal([]byte(configContent), &c)
	if err != nil {
		log.Error("parse config file", cfg, "error:", err.Error())
	}
